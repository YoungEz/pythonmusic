...
async def falar(ctx, *, arg: str=None):
    if not arg:
        return await bot.say('{} Você Precisa Falar Algo Para Eu Falar.'.format(ctx.message.author.mention))
    else:
...
async def deathnote(ctx, user: discord.User=None,*, causa: str=None):
    ...
    if not user:
        ...
        hugemb = discord.Embed(title='Faliceu',  description='**{}** morreu apos um ataque cardiaco depois de ter seu nome escrito no Death Note de **{}**'.format(user.name, ctx.message.author.name), color=0xA7ffbb)
        hugemb.set_footer(text ='Comando pedido por: {} | Shiryu Bot Oficial'.format(ctx.message.author.name))
        await bot.say(embed=hugemb)
