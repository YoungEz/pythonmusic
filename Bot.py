...
async def on_message(message):
	...
	prefix = "s!"
	comando = "mcskin"
	if message.content.lower().startswith(prefix+comando):
		prefic = len(prefix+comando) + 1
		messagem = message.content[prefic:]
		embed = discord.Embed(color=COR, )
		embed.set_image(url=f"https://mc-heads.net/skin/{messagem}")
