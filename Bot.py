...
async def racet(ctx, user: discord.User):
	user1 = (user.name), (user.name)
	user2 = (ctx.message.author.name), (ctx.message.author.name)
	embed = discord.Embed(title='Corrida', description='{} Esta Apostando Uma Corrida com {}'.format(ctx.message.author.mention, user.mention), color=0x00000f)
	await bot.say(embed=embed)
	await asyncio.sleep(2)
	user1 = (user.name), (user.name)
	user2 = (ctx.message.author.name), (ctx.message.author.name)
	race1 = random.choice(user1, user2)
	embed1 = discord.Embed(title='Sobre a corrida:', description=':rancing_car: :fire:**{}** Bateu e explodiu\n'
	':rancing_car: **{}** Venceu!'.format(race1))
