...
bot = commands.Bot(prefix, owner_id=528782261315698718)
       ...
       try:
          ...
          if rpg is None:
             await ctx.send(f"🎲 {ctx.author.mention}, você não está registrado no sistema, use s!registro para se registrar!")
          	...
          	return await ctx.send(f"<a:money:581670005306032130> | {ctx.author.mention}, Ganhou a aposta e recebeu `{aposta}` Ryucoins.")
          	if flip ==2:
          		await msg.delete()
          		moedas = int(moedas["coins"])
          		...
          		await ctx.send(f"<a:money:581670005306032130> | {ctx.author.mention}, Perdeu a aposta e perdeu `{aposta}` Ryucoins.")
       except Exception as e:
           await ctx.send(f"[Erro] {e}")
