...
async def on_ready():
    ...
    while True:
    	await bot.change_presence(game=discord.Game(name='Fui criado pelo Mateusᶠᶸᶜᵏᵧₒᵤ#6548| s!ajuda'.format(len(bot.servers)), type=2))
...
async def diversaoajuda(ctx):
    ...
    embed.add_field(name="s!falar", value="como usar ``s!userinfo @usuário`` Faça eu falar alguma coisa!", inline=True)
